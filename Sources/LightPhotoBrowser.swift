...
public protocol LightPhotoBrowserControllerPageDelegate: class {
    func lightPhotoBrowser(_ browser: LightPhotoBrowser, didMoveToPage page: Int)
}

public protocol LightPhotoBrowserControllerDismissalDelegate: class {
    func lightPhotoBrowserWillDismiss(_ browser: LightPhotoBrowser)
}

public protocol LightPhotoBrowserControllerTouchDelegate: class {
    func lightPhotoBrowser(_ browser: LightPhotoBrowser, didTouch image: LightPhotoImage, at index: Int)
}

open class LightPhotoBrowser: UIViewController {
    // MARK: - Internal views
    ...
    open fileprivate(set) var currentPage = 0 {
        didSet {
            ...
            pageDelegate?.lightPhotoBrowser(self, didMoveToPage: currentPage)
    ...
    open var images: [LightPhotoImage] {
        get {
    ...
    open weak var pageDelegate: LightPhotoBrowserControllerPageDelegate?
    open weak var dismissalDelegate: LightPhotoBrowserControllerDismissalDelegate?
    open weak var imageTouchDelegate: LightPhotoBrowserControllerTouchDelegate?
    open internal(set) var presented = false
    open fileprivate(set) var seen = false

    lazy var transitionManager: LightPhotoTransition = LightPhotoTransition()
    var pageViews = [PageView]()
    var statusBarHidden = false

    fileprivate let initialImages: [LightPhotoImage]
    fileprivate let initialPage: Int

    // MARK: - Initializers

    public init(images: [LightPhotoImage] = [], startIndex index: Int = 0) {
    ...
    open override func viewDidLoad() {
        ...
        transitionManager.lightPhotoBrowser = self
    ...
    func configurePages(_ images: [LightPhotoImage]) {
...
extension LightPhotoBrowser: UIScrollViewDelegate {
...
extension LightPhotoBrowser: PageViewDelegate {
    ...
    func pageView(_ pageView: PageView, didTouchPlayButton videoURL: URL) {
        LightPhotoConfig.handleVideo(self, videoURL)
    }

    func pageViewDidTouch(_ pageView: PageView) {
        guard !pageView.hasZoomed else { return }

        imageTouchDelegate?.lightPhotoBrowser(self, didTouch: images[currentPage], at: currentPage)
...
extension LightPhotoBrowser: HeaderViewDelegate {
    ...
    func headerView(_ headerView: HeaderView, didPressCloseButton closeButton: UIButton) {
        ...
        presented = false
        dismissalDelegate?.lightPhotoBrowserWillDismiss(self)
...
extension LightPhotoBrowser: FooterViewDelegate {
